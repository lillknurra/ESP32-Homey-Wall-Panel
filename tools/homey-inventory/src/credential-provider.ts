...
export type SafeStatusExecFile = (file: string, args: readonly string[]) => Promise<number>;
...
const realStatusExec: SafeStatusExecFile = async (file, args) => {
  try {
    await execFileAsync(file, [...args], { encoding: "utf8", maxBuffer: 1024 });
    return 0;
  } catch (error) {
...
export async function checkCredentialEntryExists(
  config: PrivateLiveConfig,
  execute: SafeStatusExecFile = realStatusExec,
): Promise<boolean> {
  if (config.credential_provider === "environment") {
    throw new CandidateError("CONFIGURATION", "Environment provider is refused during controlled preflight");
  }
  const status = await execute("/usr/bin/security", [
    "find-generic-password", "-s", config.keychain_service!, "-a", config.keychain_account!,
  ]);
  return status === 0;
}
