...
static esp_err_t publish(phone_prov_record_v1_t *r){esp_err_t e=write_record(STAGING,r);if(e!=ESP_OK)return e;phone_prov_record_v1_t v={0};bool p=false;e=read_record(STAGING,&v,&p);if(e!=ESP_OK||!p||memcmp(&v,r,sizeof(v))!=0){zero_secure(&v,sizeof(v));return ESP_ERR_INVALID_CRC;}ESP_LOGI(TAG,"PHONE_PROV persist_stage result=pass");e=write_record(ACTIVE,&v);if(e==ESP_OK){phone_prov_record_v1_t a={0};bool ap=false;e=read_record(ACTIVE,&a,&ap);if(e==ESP_OK&&ap&&memcmp(&a,&v,sizeof(a))==0){nvs_handle_t h;if(nvs_open(NS,NVS_READWRITE,&h)==ESP_OK){(void)erase_key(h,STAGING);(void)nvs_commit(h);nvs_close(h);}s_active=a;s_ctx.state=PHONE_PROV_READY;ESP_LOGI(TAG,"PHONE_PROV persist_active result=pass");s_persistence_restored=true;render_homey_display();}else zero_secure(&a,sizeof(a));}zero_secure(&v,sizeof(v));return e;}
esp_err_t phone_provisioning_boot_restore(void){phone_prov_init(&s_ctx);phone_prov_record_v1_t r={0};bool p=false;esp_err_t e=read_record(ACTIVE,&r,&p);if(e==ESP_OK&&p){s_active=r;s_generation=r.generation+1;s_ctx.state=PHONE_PROV_READY;s_persistence_restored=true;s_persistence_restored=true;ESP_LOGI(TAG,"PHONE_PROV boot_restore result=pass");render_homey_display();return ESP_OK;}if(e==ESP_OK){s_ctx.state=PHONE_PROV_UNPROVISIONED;s_persistence_restored=false;render_homey_display();return ESP_OK;}s_ctx.state=PHONE_PROV_REPROVISION_REQUIRED;show("Installation misslyckades","Radera och konfigurera igen");return e;}
