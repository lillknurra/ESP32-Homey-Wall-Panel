...
static bool s_live_runtime_ready;
...
void phone_provisioning_show_live_ready(const char *homey_name)
{
    ...
    if (!s_wifi_online) {
        s_live_runtime_ready = false;
        ESP_LOGI(TAG, "PHONE_PROV live_display_ready deferred=true wifi_online=false");
        return;
    }

    s_live_runtime_ready = true;
    show(
        s_live_homey_name,
        "Status: Ansluten");
...
static esp_err_t change_post(httpd_req_t*r){s_live_runtime_ready=false;
...
static bool light_active_homey_id(char *out, size_t capacity)
{
    ...
    out[0] = '\0';
    esp_err_t err =
        athom_oauth_runtime_get_selected_homey_id(out, capacity);
    if (err != ESP_OK) {
        out[0] = '\0';
        return false;
    }
...
static esp_err_t light_bindings_get(httpd_req_t *r)
{
    ...
    if (!access_allowed) {
        return friendly_error(
            r,
            "Favoritlampor är inte tillgängliga",
            "Panelen måste vara ansluten till den verifierade Homey-sessionen.");
    }
    panel_homey_favorites_public_t favorites={0};
    if (!panel_homey_favorites_copy_public(&favorites)) return ESP_FAIL;
    ESP_RETURN_ON_ERROR(page_begin(r,"Favoritlampor"),TAG,"lights page");
    ESP_RETURN_ON_ERROR(httpd_resp_sendstr_chunk(r,"<h1>Favoritlampor</h1><p>Urvalet hämtas automatiskt från Favoriter i Homey.</p>"),TAG,"lights intro");
    for(size_t i=0U;i<PANEL_HOMEY_FAVORITE_LIMIT;i++){
        const panel_homey_favorite_public_t *item=i<favorites.count?&favorites.items[i]:NULL;
        const char *slot=i==0U?"Belysning 1":"Belysning 2";
        const char *name=item?item->name:"Ej konfigurerad";
        const char *status="Ej konfigurerad";
        if(item)status=!item->available?"Otillgänglig":(!item->onoff_known?"Okänd":(item->onoff?"Tänd":"Släckt"));
        char block[512];int n=snprintf(block,sizeof(block),"<section><h2>%s</h2><p>%s</p><p>Status: %s</p></section>",slot,name,status);
        if(n<=0||(size_t)n>=sizeof(block))return ESP_ERR_INVALID_SIZE;
        ESP_RETURN_ON_ERROR(httpd_resp_sendstr_chunk(r,block),TAG,"favorite block");
    }
    return page_end(r);
}
